...
| **Cooldown** | Batch phase at the start of every run, heater off, until the chamber is back at its baseline (within half the detection threshold) and the valve is cold enough: at the *hold target*, or — *adaptive cooling* — cooling slower than 1 K/min with the hold it would give already ≥ 6 K below the best guess of the opening point (the *minimum gap* + a 1 K buffer, so the scatter of later test runs can't undo it). Once the batch's hold temperature is fixed, only reaching it counts. Longer than 30 min: the batch stops and says which was missing. |
...
| **Adaptive cooling** | Near room temperature the valve cools ever more slowly: the TC falls fast (~2.5 min time constant) to the body temperature, then the body cools slowly (24 Sept 2026). So a cooldown also ends once the valve cools slower than 1 K/min, if the minimum gap is secured. Lets low opening points (0.25 N·m, ~40 °C) run without waiting for an unreachable floor. A cold valve at the start of a batch with nothing known yet is held where it is, not warmed to the target (warming could open a valve that opens low); once there is a guess, the target is ≥ 20 K below it, so warming up is safe. |
| **Hold temperature** | Where the valve is held before each approach: the hold target, or where adaptive cooling ended, rounded up to 0.5 K. Fixed for the batch at the first test run's cooldown (the scouts' is provisional; a find-again frees it again), and stored with the remembered opening point, so the next batch from it starts from the same state, so every averaged run starts from the same state. A later cooldown that can't reach it in 30 min stops the batch (the lab warmed up?). Every T_open is the opening point from a cold, closed valve at this temperature. |
