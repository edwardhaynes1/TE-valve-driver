...
| **Cooldown** | Batch phase at the start of every run, heater off: until the valve temperature is at the *hold temperature* and the chamber is back at its baseline (within half the detection threshold). |
| **Hold temperature** | 35 °C, or 20 K below the reference if that is lower, but never below 28 °C. Every run starts from here, so every T_open is the opening point from a cold, closed valve in the same state. |
| **Hold** | Batch phase: auto-t holds the hold temperature for 4 min once the valve temperature is within 1 K of it, so the Invar piece and the sleeve settle to the same state every run; the approach starts once the hold time is up and the chamber is *settled*. If the valve opens while holding, its opening point is below the hold temperature: the batch stops and says so. |
...
| **Stop** | The batch ends itself: precise enough (complete), scout 1 or 2 did not open, scout 2 opened before creeping three times, two test runs in a row did not open, the valve opened while holding, or a cooldown or a settle took over 30 min. |
