...
model of the valve, whose seat opens at a set temperature and throttles.
    ...
    valve   opens when Ts ≥ open_c, shuts when Ts < open_c − hysteresis
    chamber p → base · (1 + A · exp((Ts − open_c) / efold)) while open, lag TAU_P
...
class Rig:
    ...
    def advance(self, dt):
        ...
        if not self.is_open and self.Ts >= self.open_c:
            self.is_open = True
        elif self.is_open and self.Ts < self.open_c - self.hyst:
            self.is_open = False
        target = self.base
        if self.is_open:
            target *= 1 + self.a * math.exp((self.Ts - self.open_c) / self.efold)
    ...
    def run_batch(self, n_tests=3, torque=0.3, max_s=6 * 3600, dt=0.25, hook=None):
        """Run a whole batch; returns (b, all messages)."""
        b = batch.new_batch(n_tests, torque, self.now)
